        ...
        if ($user_id !== null) {
            if (array_key_exists($position_id, $positions)) {
                if (!isUserRegistered($conn, $state_id, $user_id, $position_id)) {
                    if (registerForElection($conn, $user_id, $state_id, $position_id)) {
                        echo "Successfully registered for the election!";
                        header("Location: state.php?id=$state_id");
                        exit();
                    } else {
                        echo "Error registering for the election: " . $conn->error;
                    }
                } else {
                    echo "You are already registered for this election.";
                }
            } else {
